...
void StateTransitionManager::Initialize(TrackingBullet* bullet)
{
	...
	nameTables_[tableName.c_str()].push_back(TrackingState::kStraight);
	nameTables_[tableName.c_str()].push_back(TrackingState::kTurnToTarget);
	nameTables_[tableName.c_str()].push_back(TrackingState::kTracking);
...
void StateTransitionManager::Update()
{
	...
	// 遷移処理
	if (bullet_->GetTrackingData().transitionType) {
		// 状況に応じての切り替え
		DefaultTransition();
	}
	else {
...
void StateTransitionManager::DefaultTransition()
{
	// 遷移タイマーで変更
...
void StateTransitionManager::SetUpTransition()
{
	if (transitionTimer_.IsEnd()) {
		if (currentTable_ >= nameTables_["Default"].size()) {
			currentTable_ = 0;
		}
		
		requestState_ = nameTables_["Default"][currentTable_];
		currentTable_++;
	}
